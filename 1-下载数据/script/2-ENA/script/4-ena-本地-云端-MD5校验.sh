...
# '
# 配置区
PYTHON_PATH="/home/liuyunhui/miniconda3/envs/aspera-cli-env/bin/python3"
BASE_DIR="/mnt/f/OneDrive/科研/4_代码/9-My-Toolskit/1-下载数据/script/2-ENA"
PY_SCRIPT="$BASE_DIR/python/1-本地-云端-MD5校验.py"
LOCAL_MD5="$BASE_DIR/debug/fastq_md5.txt"
CLOUD_MD5="$BASE_DIR/debug/1_8_11_13.txt"
OUTPUT_DIR="$BASE_DIR/debug/output"
DOWNLOAD_DIR="/mnt/d/aDNA_下载/reads_fastq"
...
collect_to_delete() {
  ...
  while read -r id; do
    ...
    # 直接在下载目录中查找对应的样本目录
    local sample_dir="$DOWNLOAD_DIR/$id"
    if [[ -d "$sample_dir" ]]; then
      DELETE_LIST+=("$sample_dir")
      echo "  找到待删除目录: $sample_dir"
    else
      echo "  警告: 未找到目录 $sample_dir"
    fi
  done < "$damage_file"
}
